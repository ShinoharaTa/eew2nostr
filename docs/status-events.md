...
| `finalized` | 出し切った (最終報を受けた / 続報が来なくなった) |
| `resolved` | 解除された、または有効期限が切れた |
| `cancelled` | 取り消された (誤報) |
...
1. 解除電文を受けた場合
2. 気象警報・注意報 (`weather`) のように電文がその範囲の現況を全量で載せる種別で、
   **新しい電文に載らなくなった**場合。気象庁は警報から注意報への切り替えのときに
   警報側の解除電文を出さないため、これが無いと解除しそこねる
3. `expiresAt` を過ぎた場合。竜巻注意情報のように解除電文が出ず、
   有効期限で失効する情報が該当する

いずれも `resolved` になり、利用側から見た違いは無い。

`finalized` になる経路は2つある。ひとつは最終報を受けた場合 (緊急地震速報の
`isLastInfo`)。もうひとつは、**解除という概念が無い情報が、種別ごとの寿命を
過ぎた**場合。

| 種別 | 寿命 |
|---|---|
| `eew` | 最終更新から 10 分 |
| `earthquake` | 最終更新から 1 時間 |
| `heavy-rain` | 最終更新から 3 時間 |

地震情報・記録的短時間大雨情報には解除電文が存在せず、続報が来なくなるだけなので、
これが無いと永久に `active` に残る。寿命は最終更新 (`updatedAt`) から測るため、
続報 (VXSE51 → VXSE52 → VXSE53) が来れば測り直しになる。

解除電文で終端に到達する種別 (`weather` / `tsunami` / `volcano` / `flood` /
`sediment` / `megaquake`) には寿命を設けていない。発表が続いている最中に
消えてしまうため。

**この2つの終端化は状態を進めるだけで、SNS への配信は行わない。** 受信を止めて
いた間に溜まった取り残しが一斉に投稿されるのを避けるため。
